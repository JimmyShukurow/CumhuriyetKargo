...
{
    public function incomingCargoes(){
        $data['cities'] = Cities::all();

        GeneralLog('Gelen Kargo raporu görüntülendi');
        return view('backend.reports.incoming', compact(['data']));
    }
    public function outgoingCargoes(){
        $data['cities'] = Cities::all();

        GeneralLog('Giden Kargo raporu görüntülendi');
        return view('backend.reports.outcoming', compact(['data']));
    }

    public function getIncomingCargoes(Request $request)
    {
    ...
    public function getOutcomingCargoes(Request $request)
    {
        $trackingNo = str_replace([' ', '_'], ['', ''], $request->trackingNo);
        $invoiceNumber = $request->invoiceNumber;
        $cargoType = $request->cargoType;
        ...
        $cargoes = DB::table('cargoes')
            ...
            ->where('deperture_agency_code',Auth::user()->agency_code)
