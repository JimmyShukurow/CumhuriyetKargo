...
@section('title', 'Alt Modül Düzenle')
...
@section('content')
    ...
    <div class="app-main__inner">
        ...
        <div class="main-card mb-3 card">
            <div class="card-body">
                <h5 class="card-title">ALT MODÜL DÜZENLE</h5>
                <form method="POST" action="{{ route('module.EditSubModule', ['id' => $sm->id]) }}">
                    @csrf
                    <div class="form-row">
                        <div class="col-md-6">
                            <div class="position-relative form-group">
                                ...
                                <input name="sub_name" id="sub_name" placeholder="Alt modül ismini giriniz" type="text" required
                                    value="{{ $sm->sub_name }}" class="form-control">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="position-relative form-group">
                                ...
                                <input name="must" id="must" placeholder="Alt modüle ait sıra numarasını giriniz." type="number"
                                       value="{{ $sm->must }}" class="form-control">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-6">
                            <div class="position-relative form-group">
                                ...
                                <select class="form-control" required name="module_id" id="module_id">
                                    ...
                                    @foreach ($data as $key)
                                        <option {{ $sm->module_id  == $key->id ? 'selected' : '' }} value="{{ $key->id }}">{{ $key->name }} </option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="position-relative form-group">
                                ...
                                <input name="link" id="link" placeholder="Alt modüle ait rotayı giriniz." type="text"
                                    value="{{ $sm->link }}" class="form-control">
                            </div>
                        </div>
                    </div>
